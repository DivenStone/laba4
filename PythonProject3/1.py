def parse_yaml(text):
    """Парсит YAML-текст и возвращает словарь/список."""
    lines = text.splitlines()
    cllines = []
    for line in lines:
        if not line.strip():
            continue
        # удаляем комментарий (первый # вне кавычек – считаем, что кавычек нет)
        if '#' in line:
            pos = line.index('#')
    ...
    root = None
    # стек контейнеров и соответствующих отступов
    stack = []
    otstup = []
        ...
        if conch.startswith('- '):
            ...
            if not stack:
                ...
                if isinstance(parent, list):
                    ...
                    # словарь, первый элемент списка – нужно создать список и привязать к последнему ключу
                    if parent:
                        ...
                        # пустой словарь – некорректно, но на всякий случай
                        pass
                else:
                    pass
            ...
            if ': ' in conch:
                ...
                key = conch[:-1]
                val = None   # значение будет определено позже (вложенный блок)
            ...
            # простая строка без ':' и '-', игнорируем (YAML допускает, но мы пропускаем)
            pass
...
def parse_scalar(s):
    """Преобразует строку в число/булево/null, иначе оставляет строкой."""
    if s == '':
        return ''
    lower = s.lower()
    if lower == 'true':
...
# ----------------------------------------------------------------------
# Бинарная сериализация (собственный формат, без единого import)
# ----------------------------------------------------------------------

def serialize(obj, file):
    """Записывает объект в бинарный файл с рекурсивным обходом."""
    # маркеры типов
    TYPE_NULL = 0
    TYPE_BOOL = 1
    TYPE_INT  = 2
    TYPE_FLOAT= 3   # хранится как строка (упрощение)
    TYPE_STR  = 4
    TYPE_LIST = 5
    TYPE_DICT = 6
    ...
    def _serialize(obj):
        if obj is None:
            ...
            _write_str(str(obj))   # сохраняем как строку
            ...
            for k, v in obj.items():
                if not isinstance(k, str):
                    # YAML допускает любые ключи, но в нашем парсере ключи только строки
                    k = str(k)
                _write_str(k)
                _serialize(v)
        else:
            # не поддерживается -> сохраняем как строку
            _write_bytes(bytes([TYPE_STR]))
            _write_str(str(obj))
...
# 1. Читаем YAML-файл (в явном виде, без import)
