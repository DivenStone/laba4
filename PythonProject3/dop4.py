...
import time
import json
import yamlium   # обязательно установить: pip install yamlium

# ------------------------------------------------------------
# Функция преобразования Yamlium-объектов в примитивы Python
# (необходима для библиотечного конвейера, чтобы json.dumps не падал)
# ------------------------------------------------------------
def convert_yamlium(obj):
    """
    Рекурсивно преобразует Yamlium-узлы (Key, Scalar и т.д.) в стандартные типы:
    - Key -> str
    - Scalar -> obj.value (int, float, str, bool, None)
    """
    # 1. Узлы Yamlium
...
# ------------------------------------------------------------
# ОБЯЗАТЕЛЬНОЕ ЗАДАНИЕ: ПАРСЕР YAML (полная, самодостаточная версия)
# ------------------------------------------------------------
def _parse_scalar(s):
    """Преобразование строки в число/булево/null/строку."""
    s = s.strip()
    if s == '':
        return ''
    lower = s.lower()
...
def parse_yaml(text):
    """
    Парсит YAML-текст, возвращает словарь или список.
    Формальная грамматика: отступы пробелами, ключ: значение, списки через '-'.
    """
...
# ------------------------------------------------------------
# ДОПОЛНИТЕЛЬНОЕ ЗАДАНИЕ №1: СЕРИАЛИЗАТОР JSON (ручной, без библиотек)
# ------------------------------------------------------------
def json_dumps(obj):
...
# ------------------------------------------------------------
# КОНВЕЙЕРЫ ДЛЯ СРАВНЕНИЯ
# ------------------------------------------------------------
def yaml_to_json_manual(yaml_text):
    """Самописный конвейер: парсер YAML + ручной JSON."""
    data = parse_yaml(yaml_text)
    return json_dumps(data)

def yaml_to_json_lib(yaml_text):
    """Библиотечный конвейер: Yamlium + преобразование + стандартный json."""
...
# ------------------------------------------------------------
# ДОПОЛНИТЕЛЬНОЕ ЗАДАНИЕ №4: ИЗМЕРЕНИЕ ПРОИЗВОДИТЕЛЬНОСТИ (100 итераций)
# ------------------------------------------------------------
...
def main():
    ...
    # 2. Прогрев (чтобы исключить влияние однократного кэширования)
    print("Прогрев...")
    ...
    # 5. Проверка корректности (первые 200 символов JSON)
